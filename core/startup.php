...
# ===============================
# Deny access to unauthorised IPs
# ===============================
function onlyAdmin()
{
    global $config;

    if (count($config['adminIps']) > 0) {
        $allowed = false;

        $config['adminIps'][] = 'localhost';
        $config['adminIps'][] = '127.0.0.1';
        $config['adminIps'][] = '::1';

        foreach ($config['adminIps'] as $ip) {
        ...
        if (!$allowed) {
            die('Sorry, you are not allowed to run this file.');
        }
    }
}
